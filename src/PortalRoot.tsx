...
function ProtectedModule({children}:{children:ReactNode}){const {user,account,loading}=useAuth();if(loading)return <div className="loading">Loading Canela Portal…</div>;if(!user||!account||account.portalStatus!=='ACTIVE')return <Navigate to="/login"/>;return <main className="standalone-module">{children}</main>}
